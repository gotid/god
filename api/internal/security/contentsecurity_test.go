...
func TestContentSecurity(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			encrypter, err := codec.NewRsaEncryptor([]byte(publicKey))
			if err != nil {
				log.Fatal(err)
			}
			...
			r.Header.Set("X-Content-Security", strings.Join([]string{
				fmt.Sprintf("key=%s", fingerprint(publicKey)),
			...
			dec, err := codec.NewRsaDecryptor(file)
			assert.Nil(t, err)

			header, err := ParseContentSecurity(map[string]codec.RsaDecryptor{
				fingerprint(publicKey): dec,
			}, r)
