	...
	for _, test := range tests {
		t.Run(test.url, func(t *testing.T) {
			...
			func() {
				...
				assert.Nil(t, err)
				decrypter, err := codec.NewRsaDecryptor(keyFile)
				assert.Nil(t, err)
				contentSecurityHandler := ContentSecurityHandler(map[string]codec.RsaDecryptor{
...
func buildRequest(rs requestSettings) (*http.Request, error) {
	...
	encrypter, err := codec.NewRsaEncryptor([]byte(pubKey))
	if err != nil {
		log.Fatal(err)
	}
	...
	if !rs.missHeader {
		r.Header.Set(httpx.ContentSecurity, strings.Join([]string{
			fmt.Sprintf("fingerprint=%s", rs.fingerprint),
