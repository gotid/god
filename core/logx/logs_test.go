...
func (w *mockWriter) Info(v interface{}, fields ...LogField) {
	w.lock.Lock()
	defer w.lock.Unlock()
	output(&w.builder, levelInfo, v, fields...)
}

func (w *mockWriter) Alert(v interface{}) {
	w.lock.Lock()
	defer w.lock.Unlock()
	output(&w.builder, levelAlert, v)
}

func (w *mockWriter) Error(v interface{}, fields ...LogField) {
	w.lock.Lock()
	defer w.lock.Unlock()
	output(&w.builder, levelError, v, fields...)
}

func (w *mockWriter) Severe(v interface{}) {
	w.lock.Lock()
	defer w.lock.Unlock()
	output(&w.builder, levelSevere, v)
}

func (w *mockWriter) Slow(v interface{}, fields ...LogField) {
	w.lock.Lock()
	defer w.lock.Unlock()
	output(&w.builder, levelSlow, v, fields...)
}

func (w *mockWriter) Stack(v interface{}) {
	w.lock.Lock()
	defer w.lock.Unlock()
	output(&w.builder, levelError, v)
}

func (w *mockWriter) Stat(v interface{}, fields ...LogField) {
	w.lock.Lock()
	defer w.lock.Unlock()
	output(&w.builder, levelStat, v, fields...)
}

func (w *mockWriter) Contains(text string) bool {
	w.lock.Lock()
	defer w.lock.Unlock()
	return strings.Contains(w.builder.String(), text)
}
