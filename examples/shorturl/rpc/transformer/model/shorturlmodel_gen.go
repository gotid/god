...
var (
	...
	shorturlRowsExpectAutoSet   = strings.Join(stringx.Remove(shorturlFieldNames), ",")
	shorturlRowsWithPlaceHolder = strings.Join(stringx.Remove(shorturlFieldNames, "`shorten`"), "=?,") + "=?"
...
func newShorturlModel(conn sqlx.Conn, c cache.Config) *defaultShorturlModel {
...
func (m *defaultShorturlModel) Insert(ctx context.Context, data *Shorturl) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.Conn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Shorten, data.Url, data.CreateTime)
	}, shorturlShortenKey)
	return ret, err
}
...
func (m *defaultShorturlModel) Delete(ctx context.Context, shorten string) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.Conn) (result sql.Result, err error) {
...
func (m *defaultShorturlModel) Update(ctx context.Context, data *Shorturl) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.Conn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Url, data.CreateTime, data.Shorten)
	}, shorturlShortenKey)
	return err
}
...
func (m *defaultShorturlModel) FindOne(ctx context.Context, shorten string) (*Shorturl, error) {
	shorturlShortenKey := fmt.Sprintf("%s%v", cacheShorturlShortenPrefix, shorten)
	var resp Shorturl
	err := m.QueryRowCtx(ctx, &resp, shorturlShortenKey, func(ctx context.Context, conn sqlx.Conn, v interface{}) error {
...
func (m *defaultShorturlModel) queryPrimary(ctx context.Context, conn sqlx.Conn, v, primary interface{}) error {
	query := fmt.Sprintf("select %s from %s where `shorten` = ? limit 1", shorturlRows, m.table)
