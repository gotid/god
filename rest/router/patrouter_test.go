...
import (
	...
	"github.com/gotid/god/rest/internal/header"
	"github.com/gotid/god/rest/pathvar"
...
func TestPatRouterHandleErrors(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.method, func(t *testing.T) {
			router := NewRouter()
			err := router.Mount(test.method, test.path, nil)
...
func TestPatRouterNotFound(t *testing.T) {
	...
	err := router.Mount(http.MethodGet, "/a/b",
...
func TestPatRouter(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.method+":"+test.path, func(t *testing.T) {
			routed := false
			router := NewRouter()
			err := router.Mount(test.method, "/a/:b", http.HandlerFunc(
				func(w http.ResponseWriter, r *http.Request) {
					routed = true
					assert.Equal(t, 1, len(pathvar.Vars(r)))
				}))
			assert.Nil(t, err)
			err = router.Mount(test.method, "/a/b/c", http.HandlerFunc(
				func(w http.ResponseWriter, r *http.Request) {
					routed = true
					assert.Nil(t, pathvar.Vars(r))
				}))
			assert.Nil(t, err)
			err = router.Mount(test.method, "/b/c", http.HandlerFunc(
				func(w http.ResponseWriter, r *http.Request) {
					routed = true
				}))
			assert.Nil(t, err)
...
func TestParseSlice(t *testing.T) {
	...
	rt := NewRouter()
	err = rt.Mount(http.MethodPost, "/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		v := struct {
...
func TestParseJsonPost(t *testing.T) {
	...
	router := NewRouter()
	err = router.Mount(http.MethodPost, "/:name/:year", http.HandlerFunc(func(
	...
	assert.Equal(t, "mike:2017:whatever:200000:shanghai:20170912", rr.Body.String())
}

func TestParseJsonPostWithIntSlice(t *testing.T) {
	r, err := http.NewRequest(http.MethodPost, "https://hello.com/mike/2017",
...
func TestParseJsonPostError(t *testing.T) {
	...
	router := NewRouter()
	err = router.Mount(http.MethodPost, "/:name/:year", http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			v := struct {
				Name     string `path:"name"`
				Year     int    `path:"year"`
...
func TestParseJsonPostInvalidRequest(t *testing.T) {
	...
	router := NewRouter()
	err = router.Mount(http.MethodPost, "/", http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			v := struct {
				Ages []int `json:"ages"`
			}{}
...
func TestParseJsonPostRequired(t *testing.T) {
	r, err := http.NewRequest(http.MethodPost, "http://hello.com/mike/2017",
...
func TestParsePath(t *testing.T) {
	r, err := http.NewRequest(http.MethodGet, "http://hello.com/mike/2017", nil)
	assert.Nil(t, err)

	router := NewRouter()
	err = router.Mount(http.MethodGet, "/:name/:year", http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			v := struct {
				Name string `path:"name"`
				Year int    `path:"year"`
	...
	assert.Equal(t, "mike in 2017", rr.Body.String())
}

func TestParsePathRequired(t *testing.T) {
	r, err := http.NewRequest(http.MethodGet, "http://hello.com/mike", nil)
	assert.Nil(t, err)

	router := NewRouter()
	err = router.Mount(http.MethodGet, "/:name/", http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			v := struct {
				Name string `path:"name"`
				Year int    `path:"year"`
...
func TestParseQuery(t *testing.T) {
	r, err := http.NewRequest(http.MethodGet, "http://hello.com/mike/2017?nickname=whatever&zipcode=200000", nil)
	assert.Nil(t, err)
...
func TestParseQueryRequired(t *testing.T) {
	r, err := http.NewRequest(http.MethodPost, "http://hello.com/mike/2017?nickname=whatever", nil)
	assert.Nil(t, err)

	router := NewRouter()
	err = router.Mount(http.MethodPost, "/:name/:year", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		v := struct {
			Nickname string `form:"nickname"`
			Zipcode  int64  `form:"zipcode"`
		}{}
...
func TestParseOptional(t *testing.T) {
	r, err := http.NewRequest(http.MethodGet, "http://hello.com/mike/2017?nickname=whatever&zipcode=", nil)
	assert.Nil(t, err)
...
func TestParseNestedInRequestEmpty(t *testing.T) {
	r, err := http.NewRequest(http.MethodPost, "http://hello.com/mike/2017", bytes.NewBufferString("{}"))
	assert.Nil(t, err)
	...
	assert.Equal(t, "mike:2017", rr.Body.String())
}
...
func TestParsePtrInRequest(t *testing.T) {
	...
	assert.Equal(t, "mike:2017:100", rr.Body.String())
}

func TestParsePtrInRequestEmpty(t *testing.T) {
	r, err := http.NewRequest(http.MethodPost, "http://hello.com/mike", bytes.NewBufferString("{}"))
	assert.Nil(t, err)
	...
	router := NewRouter()
	err = router.Mount(http.MethodPost, "/mike", http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			var v WrappedRequest
			err = httpx.Parse(r, &v)
			assert.Nil(t, err)
...
func TestParse(t *testing.T) {
	...
	assert.Equal(t, "mike:2017:whatever:200000", rr.Body.String())
}
...
func TestParseWrappedGetRequestWithJsonHeader(t *testing.T) {
	r, err := http.NewRequest(http.MethodGet, "http://hello.com/mike/2017", bytes.NewReader(nil))
	assert.Nil(t, err)
...
func TestParseWrappedHeadRequestWithJsonHeader(t *testing.T) {
	r, err := http.NewRequest(http.MethodHead, "http://hello.com/mike/2017", bytes.NewReader(nil))
	assert.Nil(t, err)
	r.Header.Set(httpx.ContentType, header.JsonContentType)
	...
	router := NewRouter()
	err = router.Mount(http.MethodHead, "/:name/:year", http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			var v WrappedRequest
			err = httpx.Parse(r, &v)
			assert.Nil(t, err)
...
func TestParseWithMissingForm(t *testing.T) {
	r, err := http.NewRequest(http.MethodPost, "http://hello.com/mike/2017?nickname=whatever",
		bytes.NewBufferString(`{"location": "shanghai", "time": 20170912}`))
	assert.Nil(t, err)
	...
	err = router.Mount(http.MethodPost, "/:name/:year", http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			err = httpx.Parse(r, &v)
			assert.NotNil(t, err)
			assert.Equal(t, "字段 zipcode 未设置", err.Error())
		}))
	assert.Nil(t, err)
...
func BenchmarkPatRouter(b *testing.B) {
	...
	router := NewRouter()
	router.Mount(http.MethodGet, "/api/:user/:name", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	}))
