...
var (
	errUnsupportedType  = errors.New("设置字段值时不支持的类型")
	errNumberRange      = errors.New("数值范围设置错误")
...
func ValidatePtr(v *reflect.Value) error {
	// 顺序非常重要，IsNil 必须在检查 Kind() 之后再调用，否则会 panic。
	if !v.IsValid() || v.Kind() != reflect.Ptr || v.IsNil() {
		return fmt.Errorf("不是一个有效的指针：%v", v)
	}
...
func parseProperty(field, tag, val string) (string, error) {
	...
	if len(segs) != 2 {
		return "", fmt.Errorf("字段 %s 的选项 %s 错误", field, tag)
	}
