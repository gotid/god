...
func TestCachedConn_GetCache(t *testing.T) {
	...
	c := NewNodeConn(dummySqlConn{}, r, cache.WithExpire(time.Second*10))
	var value string
...
func TestStat(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		var str string
		err = c.QueryRow(&str, "name", func(conn sqlx.Conn, v any) error {
			*v.(*string) = "designer"
			return nil
		})
		if err != nil {
...
func TestCachedConn_QueryRowIndex_NoCache(t *testing.T) {
	...
	c := NewConn(dummySqlConn{}, cache.Config{
		{
			Config: redis.Config{
				Host: r.Addr,
				Type: redis.NodeType,
	...
	assert.Nil(t, err)
	assert.Equal(t, "designer", str)
	val, err := r.Get("index")
	assert.Nil(t, err)
	assert.Equal(t, `"primary"`, val)
	val, err = r.Get("primary/1234")
	assert.Nil(t, err)
	assert.Equal(t, `"designer"`, val)
}
...
func TestCachedConn_QueryRowIndex_HasCache(t *testing.T) {
	...
	c := NewNodeConn(dummySqlConn{}, r, cache.WithExpire(time.Second*10),
		cache.WithNotFoundExpire(time.Second))
	...
	err = c.QueryRowIndex(&str, "index", func(s any) string {
		...
		return "primary", nil
	}, func(conn sqlx.Conn, v, primary any) error {
		*v.(*string) = "xin"
...
func TestCachedConnQueryRow(t *testing.T) {
	...
	var user string
	var ran bool
	c := NewNodeConn(&conn, r, cache.WithExpire(time.Second*30))
	err = c.QueryRow(&user, key, func(conn sqlx.Conn, v any) error {
		ran = true
		user = value
		return nil
	})
...
func TestCachedConnExec(t *testing.T) {
	...
	var conn trackedConn
	c := NewNodeConn(&conn, r, cache.WithExpire(time.Second*10))
	_, err = c.ExecNoCache("delete from user_table where id='richard'")
	assert.Nil(t, err)
...
func TestCachedConnExecDropCache(t *testing.T) {
	...
	var conn trackedConn
	c := NewNodeConn(&conn, redis.New(r.Addr()), cache.WithExpire(time.Second*30))
	assert.Nil(t, c.SetCache(key, value))
	_, err = c.Exec(func(conn sqlx.Conn) (result sql.Result, e error) {
		return conn.Exec("delete from user_table where id='richard'")
	}, key)
	assert.Nil(t, err)
	assert.True(t, conn.execValue)
	_, err = r.Get(key)
...
func TestCachedConnQueryRows(t *testing.T) {
	...
	c := NewNodeConn(&conn, r, cache.WithExpire(time.Second*10))
	var users []string
	err = c.QueryRowsNoCache(&users, "select user from user_table where id='richard'")
	assert.Nil(t, err)
