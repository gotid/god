...
// spanName 是 redis 调用的跨度名称。
const spanName = "redis"

var (
	startTimeKey          = contextKey("startTime")
	durationHook          = hook{tracer: otel.GetTracerProvider().Tracer(trace.Name)}
...
// AfterProcess 执行 redis 命令后，记录可能的慢调用、请求时长和错误次数。
...
// BeforeProcessPipeline 开启一个批量命令管道跨度。
...
// AfterProcessPipeline 执行 redis 命令管道后，记录可能的慢调用、请求时长和错误次数。
...
func formatError(err error) string {
	...
	case context.DeadlineExceeded:
		return "context deadline（上下文到期）"
	case breaker.ErrServiceUnavailable:
		return "breaker（断路器打开）"
	default:
		return "unexpected error（未知错误）"
	}
}
