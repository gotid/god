...
func TestPeriodicalExecutor_hasTasks(t *testing.T) {
	...
	executor := NewPeriodicalExecutor(time.Millisecond, newContainer(time.Millisecond, nil))
	executor.newTicker = func(d time.Duration) timex.Ticker {
		return ticker
	}
	assert.False(t, executor.hasTasks(nil))
