...
func TestTimingWheel_Drain(t *testing.T) {
	...
	tw, _ := newTimingWheelWithClock(testStep, 10, func(key, value interface{}) {}, ticker)
	defer tw.Stop()
	...
	wg.Add(3)
	tw.drainAll(func(key, val interface{}) {
		lock.Lock()
		defer lock.Unlock()
...
func TestTimingWheel_SetTimerSoon(t *testing.T) {
	...
	ticker := timex.NewFakeTicker()
	tw, _ := newTimingWheelWithClock(testStep, 10, func(key, value interface{}) {
		run.CompareAndSwap(false, true)
		assert.Equal(t, "定时任务", key)
		assert.Equal(t, "参数信息", value)
		fmt.Println(key, value)
		ticker.Done()
	}, ticker)
	defer tw.Stop()

	tw.SetTimer("定时任务", "参数信息", testStep>>1)
	ticker.Tick()
