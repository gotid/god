...
func (g *Generator) genServerGroup(ctx DirContext, proto parser.Proto, cfg *conf.Config) error {
	...
	for _, service := range proto.Service {
		...
		functions, err := g.genFunctions(proto.PbPackage, service, true)
		if err != nil {
			return err
		}
...
func (g *Generator) genServerInCompatibility(ctx DirContext, proto parser.Proto, cfg *conf.Config, c *RpcContext) error {
	dir := ctx.GetServer()
	...
	functions, err := g.genFunctions(proto.PbPackage, service, false)
	if err != nil {
		return err
	}
