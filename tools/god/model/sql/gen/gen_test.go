...
func TestCacheModel(t *testing.T) {
	...
	g, err := NewDefaultGenerator(cacheDir, &config.Config{
		NamingFormat: "GoDesigner",
	})
	assert.Nil(t, err)

	err = g.StartFromDDL(sqlFile, true, false, "go_designer")
	assert.Nil(t, err)
	...
	err = g.StartFromDDL(sqlFile, false, false, "go_designer")
	assert.Nil(t, err)
	assert.True(t, func() bool {
...
func TestFolderName(t *testing.T) {
	...
	err = g.StartFromDDL(sqlFile, true, true, "go_designer")
	assert.Nil(t, err)
	assert.True(t, func() bool {
		_, err := os.Stat(filepath.Join(camelDir, "TestUserModel.go"))
		return err == nil
