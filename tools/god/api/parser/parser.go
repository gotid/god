...
func Parse(filename string) (*spec.ApiSpec, error) {
	...
	astApi, err := astParser.Parse(filename)
	if err != nil {
		return nil, err
	}

	apiSpec := new(spec.ApiSpec)
	p := parser{ast: astApi, spec: apiSpec}
	err = p.convert2Spec()
	if err != nil {
		return nil, err
	}
...
func parseContent(content string, skipCheckTypeDeclaration bool, filename ...string) (*spec.ApiSpec, error) {
	...
	astApi, err := astParser.ParseContent(content, filename...)
	if err != nil {
		return nil, err
	}
	...
	for _, item := range p.ast.Type {
		...
		case *ast.TypeStruct:
			...
			for _, field := range v.Fields {
				members = append(members, p.fieldToMember(field))
			}
...
func (p parser) fieldToMember(field *ast.TypeField) spec.Member {
	...
	if !field.IsAnonymous {
		...
		if field.Tag == nil {
			panic(fmt.Sprintf("错误：行 %d:%d 字段 %s 没有tag标签", field.Name.Line(), field.Name.Column(),
				field.Name.Text()))
		}
...
func (p parser) findDefinedType(name string) (*spec.Type, error) {
	...
	return nil, fmt.Errorf("类型 %s 未定义", name)
}
...
func (p parser) fillRouteAtServer(astRoute *ast.ServiceRoute, route *spec.Route) error {
	if astRoute.AtServer != nil {
		...
		if len(route.Handler) == 0 {
			return fmt.Errorf("缺少 %q 的 handler 处理器注解", route.Path)
		}

		for _, char := range route.Handler {
			if !unicode.IsDigit(char) && !unicode.IsLetter(char) {
				return fmt.Errorf("路由 [%s] handler 处理器 [%s] 无效，处理器名称只应该包含字母和数字",
