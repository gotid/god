...
func genRoutes(dir, rootPkg string, cfg *config.Config, api *spec.ApiSpec) error {
	...
	for _, g := range groups {
		...
		if len(g.jwtTrans) > 0 {
			jwt = jwt + fmt.Sprintf("\n api.WithJwtTransition(serverCtx.Config.%s.PrevSecret,serverCtx.Config.%s.Secret),", g.jwtTrans, g.jwtTrans)
		}
		var signature, prefix string
		if g.signatureEnabled {
			signature = "\n api.WithSignature(serverCtx.Config.Signature),"
		}
		if len(g.prefix) > 0 {
			prefix = fmt.Sprintf(`
api.WithPrefix("%s"),`, g.prefix)
		}
		...
		if len(g.timeout) > 0 {
			...
			timeout = fmt.Sprintf("api.WithTimeout(%d * time.Millisecond),", duration/time.Millisecond)
			hasTimeout = true
		}
